...
function Interface() {
  ...
  async function startSession() {
    try {
        ...
        const objectiveMessage = 'I want to learn something!';
        ...
        try {
            ...
            if (agent_response && agent_response.length) {
                ...
                parsedMessages.forEach(parsedMessage => {
                    const messageText = parsedMessage.agent_info || parsedMessage.agent_json?.text || '';
                    ...
                    if (messageText) {
                        ...
                        setMessages(prevMessages => [
                            ...prevMessages, 
                            { message: fullMessage, options: options, person: 1 }
                        ]);
                        scrollToBottom();
                    }
                ...
                setMessages(prevMessages => [
                    ...prevMessages,
                    { message: "User selected: Learning Agent 101", person: 0 }
                ]);
                scrollToBottom();
                ...
                try {
                    ...
                    const fetchedMessages = response2.data.agent_response.map(res => {
                        ...
                        return {
                            message: parsedMessage.agent_info || parsedMessage.agent_json?.text || 'No content',
...
async function submitHandler(e) {
  ...
  return (
    <div>
      <Chat messages={messages} setMessages={setMessages} inputRef={inputRef} submitHandler={submitHandler} active={active} onContinueDialogue={handleContinueDialogue} onEndDialogue={handleEndDialogue} sessionId={sessionId} startSession={startSession} setSessionId={setSessionId} learn={true}/>
    </div>
  );
}
